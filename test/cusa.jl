...
using QuantumAdiabaticAnnealing: linear_to_cartesian, cartesian_to_linear, hasparent, child_nodes, child_logic, SimulatedAnnealingHamiltonian,
    parent_logic, random_state, natom, track_equilibration!, step!, HeatBath, Metropolis
...
if CUDA.functional()
    @testset "sa (gpu)" begin
        ...
        @testset "Metropolis" begin
            ...
            @test step!(Metropolis(), sa, state, 1.0, 1) isa Matrix
            @test track_equilibration!(Metropolis(), sa, state) isa SimulatedAnnealingHamiltonian
        end
        @testset "HeatBath" begin
            state = CuArray(random_state(sa, nbatch))
