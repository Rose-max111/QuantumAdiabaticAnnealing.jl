...
mutable struct SimulatedAnnealingMIS
    ...
    const a::Vector{Float64}   # chemical potential (i.e. single-site energy)
    const b::Float64   # blockade energy

    # internal states of the annealing process
    IS_bitarr::Vector{Bool}
    best_IS_bitarr::Vector{Bool}
    # obj means objective
    obj::Float64
    best_obj::Float64

    function SimulatedAnnealingMIS(graph::SimpleGraph; a::Vector{Float64}, b::Float64 = Inf)
        # internal states of the annealing process
        IS_bitarr = zeros(Bool, nv(graph))
        best_IS_bitarr = zeros(Bool, nv(graph))
        new(graph, a, b, IS_bitarr, best_IS_bitarr, 0.0, 0.0)
    end
end
...
function track_equilibration!(sa::SimulatedAnnealingMIS, target_obj::Float64, depth::Int, tempscale = 4 .- (1:100 .- 1) .* 0.04, niters = 4000)
        ...
        for runtime in 1:niters
            ...
            for v in mynodelist
                ...
                if sa.obj < sa.best_obj
                    sa.best_obj = sa.obj
                    sa.best_IS_bitarr = copy(sa.IS_bitarr)
                end
                
                # Once we reach the target energy, switch off
