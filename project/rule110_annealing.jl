...
lines!(ax, delta_over_omega, [(val[2] - val[1]) for val in eigvals], color = :blue, linewidth = 2, label = "ED FE vs GS(deterministic direction)")
lines!(ax, delta_over_omega, [(val[2] - val[1]) for val in eigvals1], color = :red, linewidth = 2, label = "ED SE vs GS(non-deterministic direction)")
